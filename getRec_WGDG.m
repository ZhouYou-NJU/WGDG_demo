function [ImgRec] = getRec_WGDG(RawImgSet, LEDBoard, MainPara, Delta_g, DataLabel)
% this fucntion realizes the reconstruction code of WGDG method in the paper
...
ResultsFolder                 = sprintf('WGDG_result_%s_%s',DataLabel,ResultsFolder);
mkdir(ResultsFolder);
%% Initialization
...
for iIte = 1 : nIterative
    for iImg = 1 : nImg
        ...
        title_name       = sprintf('WGDG, Iterative = %d, Imgae Num = %d',iIte,iImg);
    ...
    AmpImgRecName        = sprintf('WGDG AmpImgRec Iterative= %d.png',iIte);
    PhsImgRecName        = sprintf('WGDG PhsImgRec Iterative= %d.png',iIte);
